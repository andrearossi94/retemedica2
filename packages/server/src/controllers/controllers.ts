    ...
    try{
        ...
        let adp = await getAdapter(query.user, query.org);
            res.status(200).send(await ClientFactory(CartellaclinicaController, adp)
                .create(params.cartellaclinica, params.username, params.password));
...
export async function CartellaclinicaController_degenza_post(req: Request, res: Response): Promise<void>{
    try{
        ...
        let adp = await getAdapter(query.user, query.org);
            res.status(200).send(await ClientFactory(CartellaclinicaController, adp)
                .degenza(params.id, params.username, params.password));
...
export async function CartellaclinicaController_cambiaconsenso_post(req: Request, res: Response): Promise<void>{
    try{
        ...
        let adp = await getAdapter(query.user, query.org);
            res.status(200).send(await ClientFactory(CartellaclinicaController, adp)
                .cambiaconsenso(params.id, params.username, params.password));
...
export async function CartellaclinicaController_get_post(req: Request, res: Response): Promise<void>{
    try{
        let params = req.body;
        let query = req.query;
        let adp = await getAdapter(query.user, query.org);
        res.status(200).send(await ClientFactory(CartellaclinicaController, adp).get(params.id, params.username, params.password));
...
export async function CartellaclinicaController_getByUsername_post(req: Request, res: Response): Promise<void>{
    try{
        let params = req.body;
        let query = req.query;
        let adp = await getAdapter(query.user, query.org);
        res.status(200).send(await ClientFactory(CartellaclinicaController, adp)
            .getByUsername(params.pazienteid, params.username, params.password));
